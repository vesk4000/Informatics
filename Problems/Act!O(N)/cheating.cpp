///Modified copy of dijkstraReadable.cpp
#include <iostream>
#include <vector>
#include <limits>
#include <sstream>
	...
	while(!toBeEvaled.empty()) {
		...
		if(used[currNode] == false) {
			...
			for(int i = 0;i < nb[currNode].size(); ++i) {
				...
				if( used[currNb.node] == false && dist[currNode] + currNb.cost < dist[ currNb.node ]) {
...
void output() {
	if(used[numNodes] == false)
		cout << -1 << endl;
	else
		cout << dist[numNodes] << endl;
	/*cout << "node used dist path" << endl;
	for(int i = 1;i <= numNodes; ++i) {
		cout << i << ": " << used[i] << "; " << dist[i] << "; " << restorePath(i) << ";\n";
	}
	for(int i = 0;i < nb[5].size(); ++i)
	{
		cout << 5 << " " << nb[5][i].node << " " << nb[5][i].cost << endl;
	}
	cout << dist[2] << endl;*/
}
